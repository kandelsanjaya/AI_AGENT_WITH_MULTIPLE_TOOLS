...
def generate_analytical_pdf(content: str, image_bytes_list: List[bytes]) -> bytes:
    """Generate a PDF report that includes text content and images (charts)."""
    try:
        from fpdf import FPDF
        import tempfile
        ...
        class StyledPDF(FPDF):
            def header(self):
                self.set_fill_color(34, 197, 94) # Green accent for data
                self.rect(0, 0, 7, 297, "F")
                self.set_fill_color(30, 41, 59) 
                self.rect(7, 0, 203, 14, "F")
                self.set_font("Helvetica", "B", 7.5)
                self.set_text_color(226, 232, 240)
                self.text(12, 8, "EDUSPHERE AI - DATA ANALYST REPORT")
            ...
            def footer(self):
                ...
                self.set_font("Helvetica", "I", 7.5)
                self.set_text_color(100, 116, 139)
                self.text(130, 290, f"Page {self.page_no()} | Generated by AI Data Analyst")
        ...
        if image_bytes_list:
            ...
            for img_bytes in image_bytes_list:
                ...
                # Check if we need a new page for the image (approx height 100)
                if pdf.get_y() > 180:
                    pdf.add_page()
                
                try:
                    pdf.image(tmp_path, x=15, w=180)
                    pdf.ln(10) # spacing after image
                finally:
                    try:
                        os.unlink(tmp_path)
                    except:
        ...
        log.error("Analytical PDF generation failed: %s", exc)
        return content.encode("latin-1", "replace")

def get_analytical_pdf_download_link(content: str, image_bytes_list: List[bytes], filename: str, label: str) -> str:
    pdf_bytes = generate_analytical_pdf(content, image_bytes_list)
    b64 = base64.b64encode(pdf_bytes).decode("utf-8")
    return (
        f'<a href="data:application/pdf;base64,{b64}" '
        f'download="{filename}" class="download-btn">📄 {label}</a>'
